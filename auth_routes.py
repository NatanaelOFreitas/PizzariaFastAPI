...
async def criar_conta(usuario_schema: Usuario_schema, session: Session = Depends(pegar_session)):
    ...
    if usuario:
        ...
        session.commit()
        return {
            "mensagem": f"Usuário cadastrado com sucesso, novo usuário: {usuario_schema.email}"
            }
